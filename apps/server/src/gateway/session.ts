...
export class GatewaySession {
	...
	readonly #acquireTurn: () => (() => void) | null;
	...
	#helloAccepted = false;
	#resumeTokenHash: Uint8Array | null = null;
	#stopped = false;
	...
	attach(socket: GatewaySocket): void {
		...
		// Binding happens only after a validated client.hello.
	}
	...
	detach(socket: GatewaySocket): void {
		...
		this.#assembler = null;
		void this.#orchestrator.disconnect();
	...
	async stop(
		...
		if (this.#currentGenerationId) {
			await this.#orchestrator
				.interrupt(this.#currentGenerationId)
				.catch(() => undefined);
		}
		await Promise.allSettled([...this.#activeProcesses]);
	...
	async #receiveJson(socket: GatewaySocket, value: unknown): Promise<void> {
		if (this.#activeSocket !== socket) {
			...
			if (this.#activeSocket && this.#activeSocket !== socket) {
				this.#activeSocket.close(1000, "session resumed elsewhere");
			}
	...
	async #commitAudio(): Promise<void> {
		...
		this.#assembler = null;
		const release = this.#acquireTurn();
		if (!release) {
	...
	#acceptResumeToken(token: string | null): boolean {
		...
		if (!token || !this.#resumeTokenHash) return false;
	...
	#persistTurn(turnId: string): void {
		...
		if (!turn?.userText || this.#persistedTurns.has(turn.id)) return;
