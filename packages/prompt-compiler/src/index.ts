...
async function assertNoSymlinkComponents(path: string): Promise<void> {
  ...
  for (const component of path.slice(root.length).split(sep).filter(Boolean)) {
    ...
    if (!stat) return;
    if (stat.isSymbolicLink()) fail('runtime directory path must not contain symlinks');
  }
}
...
async function assertRuntimeOutsideSource(
  ...
  assertOutside(canonicalSourceRoot, await resolvePotentialRealPath(runtimeDir));
}

async function readRegularSourceFile(path: string, label: string): Promise<string> {
  const parentStat = await lstat(dirname(path)).catch(() => undefined);
  if (!parentStat?.isDirectory())
    fail(`source directory for ${label} must be a regular directory, not a symlink`);
  const stat = await lstat(path).catch(() => undefined);
  if (!stat) fail(`missing required file: ${label}`);
  if (!stat.isFile()) fail(`${label} must be a regular file, not a symlink or directory`);
  if (stat.size > MAX_FILE_BYTES) fail(`${label} exceeds ${MAX_FILE_BYTES} bytes`);
...
function validateSource(relativePath: string, source: string): string {
  ...
  const headings = [...normalized.matchAll(/^(#{1,6})\s+(.+?)\s*$/gmu)].map((match) => ({
    level: match[1].length,
    text: match[2],
  }));
  const expected = HEADINGS[relativePath];
  const expectedLevels =
    HEADING_LEVELS[relativePath] ?? expected?.map((_heading, index) => (index === 0 ? 1 : 2));
  if (
    !expected ||
    headings.length !== expected.length ||
    headings.some(
      (heading, index) =>
        heading.text !== expected[index] || heading.level !== expectedLevels?.[index],
    )
  ) {
    fail(`${relativePath} has unexpected headings, heading levels, or heading order`);
  }
...
async function prepareRuntime(runtimeDir: string): Promise<void> {
  ...
  for (const entry of entries) {
    if (entry.name !== 'AGENTS.md' || entry.isSymbolicLink() || !entry.isFile()) {
      fail('runtime directory contains unexpected contents or symlinks');
    }
  }
}

export async function compilePromptBundle(options: CompileOptions): Promise<CompileMetadata> {
  ...
  for (const relativePath of PROMPT_ORDER) {
    ...
    const source = validateSource(relativePath, await readRegularSourceFile(path, relativePath));
    chunks.push(`<!-- BEGIN ${relativePath} -->\n`, source, `<!-- END ${relativePath} -->\n\n`);
  }
  ...
  const tempPath = resolve(dirname(runtimeDir), `.AGENTS.md.${process.pid}.${randomUUID()}.tmp`);
