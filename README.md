...
Luna remains fixed at reasoning effort `low`, its lowest supported setting; reasoning cannot be disabled. `CODEX_SERVICE_TIER` defaults to empty standard service for portability. A local subscription owner may set exact `priority` for the Codex-advertised Fast tier (1.5x speed with increased subscription usage), but this provides no latency SLA. Readiness fails closed if exact Luna does not advertise the requested tier.
