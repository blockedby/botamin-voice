...
- Every thread and turn uses `approvalPolicy: never`, read-only sandbox with network denied, one isolated runtime workspace root and the configured model/effort; the release profile remains `gpt-5.6-luna` with `low`. Luna advertises only `low|medium|high|xhigh|max`; reasoning cannot be disabled and `low` is the lowest supported setting. Thread start/resume also pins `modelProvider: openai`, disables provider model fallback, and verifies the returned provider. Readiness verifies the effective `config/read` provider before checking subscription auth and exact-model availability. `environments` is omitted only on thread start/resume because CLI 0.146.0 returns no `instructionSources` when an empty environment list is supplied; every model turn explicitly sends `environments: []`, after instructions have been verified.
- `CODEX_SERVICE_TIER` is closed to empty/unset (portable standard service) or exact `priority`. When selected, the immutable provider option sends `serviceTier: "priority"` on every `turn/start`; visitor/model data cannot select it, and it is not inherited by the child environment. Readiness fails closed with `CODEX_MODEL_OR_TIER_UNAVAILABLE` unless the exact selected Luna catalog entry advertises service-tier id `priority`. Pinned CLI 0.146.0 describes that tier as Fast, 1.5x speed with increased subscription usage. This is provider-advertised routing, not a latency SLA.
...
Output is metadata only: CLI version, configured model, health, prompt hash, instruction-source verification, whether a provider text delta streamed, whether addressable interrupt was sent, raw terminal status, completion and the fixed security profile. Preflight uses safe envelope mode and observes provider-delta metadata without printing model text. It never prints auth data, prompts or model text. If standard service is selected and `gpt-5.6-luna`/`low` is absent, health returns `CODEX_MODEL_OR_EFFORT_UNAVAILABLE`; requested priority instead returns `CODEX_MODEL_OR_TIER_UNAVAILABLE` when the exact model or tier advertisement is absent. No other model or tier is substituted.
