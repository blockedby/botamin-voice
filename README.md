...
Sixteen committed same-origin reaction MP3s may play once per eligible turn after a 350 ms delay, but only after capability negotiation and conservative server policy. They do not alter transcript, state, provider selection, or business effects and make zero runtime provider calls. Their corpus is already committed; regenerating it is an explicit paid administrator opt-in. This does not change the separately allowed static same-origin proactive greeting, and no conversation/provider behavior occurs before both consents.
